import { slideArr, PagenationBtnType } from "./Interface";

function Pagenation({ slideIndex, setSlideIndex }: PagenationBtnType) {
  const slideLength = slideArr.length;

  // 개수를 동적으로 변경하기 위해서 slideLength만큼의 배열 원소를 가지는 배열 생성
  let pagenationArr = new Array(slideLength);
  pagenationArr.fill(1);
  ...
  const pagenationHandler = (index: number) => {
      ...
      <ol className="flex">
        {pagenationArr.map((item, index) => (
          <li
            ...
            key={index}
            onClick={() => pagenationHandler(index)}
          ></li>
        ))}
      </ol>
    </div>
